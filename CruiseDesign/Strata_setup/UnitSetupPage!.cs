      ...
      private void checkUOMfield()
      {
       ...
       private void useReconStrataToolStripMenuItem_Click(object sender, EventArgs e)
        ...
        private bool checkStrata()
        {
           ...
           if (dataGridViewStratum.RowCount > 0)
           {
              if (MessageBox.Show("Strata already exists.\nDo you wish to replace existing Strata with Recon Strata definitions?", "Warning", MessageBoxButtons.YesNo) == DialogResult.No)
              {
                 return (false);
              }
           }
